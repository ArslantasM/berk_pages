...
// Fix broken intra-language navigation in folder-based translations (e.g. /ko/, /hi/, /en/)
// Many translated HTML files still contain links like "ko_installation.html" which don't exist
// in the folder export (actual files are "installation.html", etc.).
(function fixFolderLanguageLinks() {
    const pathname = (window.location && window.location.pathname ? window.location.pathname : '').replace(/\\/g, '/');
    const match = pathname.match(/\/(ko|hi|en)\//);
    if (!match) {
        return;
    }
    ...
    function rewriteHref(originalHref) {
        if (!originalHref) {
            return originalHref;
        }

        // Ignore in-page anchors and absolute URLs
        if (
            originalHref.startsWith('#') ||
            originalHref.startsWith('mailto:') ||
            originalHref.startsWith('tel:') ||
            /^(?:[a-z+]+:)?\/\//i.test(originalHref)
        ) {
            return originalHref;
        }

        const hashSplit = originalHref.split('#');
        const beforeHash = hashSplit[0];
        const hash = hashSplit.length > 1 ? `#${hashSplit.slice(1).join('#')}` : '';

        const querySplit = beforeHash.split('?');
        let path = querySplit[0];
        const query = querySplit.length > 1 ? `?${querySplit.slice(1).join('?')}` : '';

        // Only rewrite local HTML links
        if (!path.endsWith('.html')) {
            return originalHref;
        }

        // Strip the language prefix at the beginning of any path segment
        // Examples:
        // - ko_installation.html -> installation.html
        // - basics/ko_syntax.html -> basics/syntax.html
        path = path.replace(new RegExp(`(^|/)${prefix}`,'g'), '$1');

        return `${path}${query}${hash}`;
    }

    function rewriteElements(selector, attr) {
    ...
    // Links in content, next/prev chapter nav, and print link are all <a href>
