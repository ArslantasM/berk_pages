# BERK Programlama Dili - Dokümantasyon ve İndirme

Bu repository, BERK Programlama Dili'nin resmi dokümantasyonu ve indirme dosyalarını içerir.

## 📚 Dokümantasyon

Tam dokümantasyon için GitHub Pages'i ziyaret edin:  
**https://arslantasm.github.io/berk_pages/**

## 📥 İndirme

### BERK Derleyici v0.8.0

**Windows x64:**
- [BERK-lang.exe](./releases/BERK-lang.exe) (~4.5 MB)

**Özellikler:**
- Çift dilli syntax (Türkçe/İngilizce)
- LLVM Backend
- C/C++ seviyesinde performans
- 19 hedef platform desteği

## 🚀 Hızlı Başlangıç

```bash
# 1. BERK-lang.exe'yi indirin
# 2. PATH'e ekleyin

# İlk programınız
fonksiyon ana() -> tamsayı
yap
    yazdır("Merhaba, BERK!")
    dön 0
son

# Derleyin ve çalıştırın
BERK-lang merhaba.berk -o merhaba.exe
```

## 📖 İçerik

Bu repository şunları içerir:

- **Tam HTML Dokümantasyon** - mdBook ile oluşturulmuş etkileşimli kitapçık
- **BERK Derleyici** - Windows x64 executable
- **Örnek Kodlar** - Dokümantasyon içinde

## 🔗 Bağlantılar

- **Ana Repo (Private):** [ArslantasM/berk](https://github.com/ArslantasM/berk)
- **Dokümantasyon:** [GitHub Pages](https://arslantasm.github.io/berk_pages/)

## 📝 Lisans

BERK Programlama Dili ve dokümantasyonu ArslantasM tarafından geliştirilmektedir.

---

**Son Güncelleme:** 19 Kasım 2025  
**Versiyon:** 0.8.0
